<style>
  ...
  .nav-pills>li.active>a, .nav-pills>li.active>a:focus, .nav-pills>li.active>a:hover, .panel-info>.panel-heading{
        ...
        while ($row = mysqli_fetch_array($run_query)) {
            ...
            <div class='col-md-4'>
                    <div class='panel panel-info' >
                        <div class='panel-heading' style='background-color:black; color:white;'>$pro_title</div>
                        <div class='panel-body' > <img src='product_images/$pro_image' style='width:210px; height:250px;margin-bottom:1vh;'/></div>
                        
                        <div class='panel-heading' style='background-color:black; color:white;' >$.$pro_price.00
                        <button pid='$pro_id' id='product' style='float:right;' style='background-color:white!important; color:black!important;' class='btn btn-danger btn-xs'>Add this to cart</button>
                    </div>
                    </div>
                    </div>
    ...
    while($row = mysqli_fetch_array($run_query)){
            ...
            <div class='col-md-4'>
                    <div class='panel panel-info'>
                        ...
                        <div class='panel-body'> <img src='product_images/$pro_image' style='width:210px; height:250px;margin-bottom:1vh;'/></div>
                        <div class='panel-heading' style='background-color:black; color:white;'>$.$pro_price.00
                        <button pid='$pro_id' id='product' style='float:right;' class='btn btn-danger btn-xs'>Add this to Cart</button>
...
if(isset($_POST["addProduct"])){
    if (isset($_SESSION["uid"])) {
        ...
        $count = mysqli_num_rows($run_query);
        if ($count   > 0) {
            echo "Already in cart, Please select a different item or edit quantity in cart!";
        }else {
        ...
        if(mysqli_query($con,$sql)) {
            ...
            <b>Item has been successfully added to cart!</b>
            </div>
            ";
        }
        }
       ...
       <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
       <b>Please log in or sign up to continue!</b>
       </div>   
       ";
    }
...
if (isset($_POST["car_checkout"])) {
        ...
        echo '    
            ...
            <input type="hidden" name="custom" value="'.$uid.'"/>
                ...
                <a href="payment_success.php" class="btn btn-success btn-lg" style="float:right;margin-top:10px;background-color:black;color:white;">Complete Payment</a>
                <hr/>
          </form>
        ';
    }
...
if($run_quey) {
    ...
    <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>Successfully removed from cart!</b>
    </div>
    ";
}
}
...
if (isset($_POST["updatproduct"])){
    ...
    if ($run_query) {
        echo "
        <div class='alert alert-success'>
        <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><b>Successfully updates!</b>
        </div>  
        ";
    }
}
